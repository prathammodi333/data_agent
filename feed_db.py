...
def main():
    if os.environ.get("ALLOW_DEMO_DB_RESET") != "YES":
        raise RuntimeError("Set ALLOW_DEMO_DB_RESET=YES only for a separate, disposable demo database")
    target_name = os.environ.get("DB_NAME") or os.environ.get("database", "")
    if "demo" not in target_name.lower() or os.environ.get("DEMO_DB_NAME_CONFIRM") != target_name:
        raise RuntimeError("Use a database named for the demo and set DEMO_DB_NAME_CONFIRM to its exact name")
    ...
    DB_CONFIG = {
        ...
        "port": int(os.environ.get('DB_PORT') or os.environ['port']),
        ...
        "sslmode": "require"  # CRITICAL: Neon requires an encrypted SSL connection
    }
